                ...
                <div class="p-4 border rounded table-responsive">
                    <table class="table table-hover table-striped align-middle">
                        ...
                        <tbody id="sortable-youtube-videos">
                            ...
                            @empty
                            <tr>
                                <td colspan="7" class="text-center text-muted py-4">No videos added yet.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        $('#bulk-delete').on('click', function() {
            ...
            if (typeof window.deleteConfirmModal !== 'undefined') {
                window.deleteConfirmModal.show(
                    ...
                    'Are you sure you want to delete ' + ids.length + ' selected video(s)? This action cannot be undone.',
                    function() {
                        $.ajax({
                            url: "{{ route('admin.youtube_videos.bulk_delete') }}",
                            method: "POST",
                            data: {
                                ids: ids,
                                _token: "{{ csrf_token() }}"
            ...
            } else if (confirm('Delete ' + ids.length + ' video(s)?')) {
                $.ajax({
                    url: "{{ route('admin.youtube_videos.bulk_delete') }}",
                    method: "POST",
                    data: {
