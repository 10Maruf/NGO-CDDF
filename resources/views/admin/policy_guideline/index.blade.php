...
@section('title_l1', 'Policy & Guidelines')
@section('bread_crumb')
    <li class="breadcrumb-item">Policy & Guidelines</li>
@endsection
    ...
    <div class="col-md-12 mx-auto">
        <div class="d-flex align-items-center justify-content-between mb-3">
            ...
            <a href="{{ route('policy.create') }}" class="btn btn-primary btn-sm">
                <i class="feather-plus me-1"></i> Add Policy
            </a>
        </div>
        <hr/>
        <div class="card">
        ...
        $('#bulk-delete').on('click', function() {
            ...
            if (typeof window.deleteConfirmModal !== 'undefined') {
                window.deleteConfirmModal.show(
                    ...
                    'Are you sure you want to delete ' + ids.length + ' selected policy item(s)? This action cannot be undone.',
                    function() {
                        $.ajax({
                            url: "{{ route('policy.bulk_delete') }}",
                            method: "POST",
                            data: {
                                ids: ids,
                                _token: "{{ csrf_token() }}"
            ...
            } else if (confirm('Delete ' + ids.length + ' policy item(s)?')) {
                $.ajax({
                    url: "{{ route('policy.bulk_delete') }}",
                    method: "POST",
                    data: {
